    ...
    try {
        ...
        if (!req.body.featuredArtists || req.body.featuredArtists == null) {
            ...
            for (const featuredArtistId of album.featuredArtists) {
                ...
                await Song.updateOne({ _id: songList._id }, { $pull: { featuredArtists: featuredArtistId } });
            ...
            for (const featuredArtistId of featuredArtists) {
                ...
                featuredArtist.albumList ? console.log("do not update album list") : featuredArtist.albumList.push(album._id);
...
const deleteAlbum = async (req, res) => {
    ...
    try {
        ...
        const featuredArtists = await Promise.all(album.featuredArtists.map(async () => {

            const featuredArtist = await Artist.findOne({ _id: album.featuredArtists });
        ...
        await Artist.updateOne({ _id: artist._id }, { $pull: { albumList: album._id, songList: { $in: album.songList } } });

        await Song.deleteMany({ _id: album.songList });

        for (const featuredArtistId of featuredArtists) {

            await Artist.updateOne({ _id: featuredArtistId }, { $pull: { albumList: album._id, songList: { $in: album.songList } } });
